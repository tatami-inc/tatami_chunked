...
protected:
    ...
    auto simple_next(Cache_& cache, int& counter, int& wave) {
        return cache.next(
...
TEST_F(OracularVariableSlabCacheTest, Consecutive) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        auto out = simple_next(cache, counter, cycle); 
...
TEST_F(OracularVariableSlabCacheTest, Reused) {
    ...
    for (size_t i = 0; i < 7; ++i) { // fetching 11 to 28.
        auto out = simple_next(cache, counter, wave); 
        EXPECT_EQ(out.first->chunk_id, static_cast<unsigned char>(predictions[i] / 10));
        EXPECT_EQ(out.first->cycle, 1);
    ...
    for (size_t i = 12, end = predictions.size(); i < end; ++i) { // fetching 24 to 15.
        ...
        if (out.first->chunk_id == 1) {
            ...
            EXPECT_EQ(out.first->populate_number, 5); // slab 2 needs to be reloaded.
            EXPECT_EQ(out.first->reuse_number, 0);
        }
...
//    }
//
//    tatami_chunked::OracularVariableSlabCache<unsigned char, int, TestSlab> cache(std::make_unique<tatami::FixedViewOracle<int> >(predictions.data(), predictions.size()), cache_size);
//    int counter = 0;
//    int nalloc = 0;
//
//    for (size_t i = 0; i < predictions.size(); ++i) {
