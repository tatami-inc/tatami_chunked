...
Index_ compute_max_slabs_in_cache(std::size_t slab_size_in_elements, Index_ num_slabs, std::size_t cache_size_in_elements, bool require_minimum_cache) {
 ...
 * This is also used as the type of the maximum number of slabs, under the assumption that there are no more slabs than the larger dimension extent.
 */
template<typename Index_>
struct SlabCacheStats {
    ...
    SlabCacheStats(Index_ target_length, Index_ non_target_length, Index_ target_num_slabs, std::size_t cache_size_in_elements, bool require_minimum_cache) :
        // Don't be tempted to do unsafe casts of target_length to size_t, as a smaller type could allow us to sip the overflow check for the product.
        // Also, this class might be used outside of the tatami::Matrix contract (i.e., Index_ might store values beyond std::size_t).
