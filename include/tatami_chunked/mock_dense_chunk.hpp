...
public:
    ...
    void extract(Index_ target_start, Index_ target_length, Index_ non_target_start, Index_ non_target_length, Workspace<value_type>& work, value_type* output, size_t stride) const {
        chunk.inflate(work);
        output += static_cast<size_t>(target_start) * stride; // cast to size_t to avoid overflow.

        if constexpr(Blob_::row_major == accrow_) {
            size_t non_target_chunkdim = get_non_target_chunkdim<accrow_>(); // use size_t to avoid integer overflow with Index_.
            ...
            size_t target_chunkdim = get_target_chunkdim<accrow_>(); // use size_t to avoid integer overflow with Index_.
    ...
    template<bool accrow_, typename Index_>
    void extract(Index_ target_start, Index_ target_length, const std::vector<Index_>& non_target_indices, Workspace<value_type>& work, value_type* output, size_t stride) const {
        chunk.inflate(work);
        output += static_cast<size_t>(target_start) * stride; // cast to size_t to avoid overflow.
...
public:
    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& target_indices, Index_ non_target_start, Index_ non_target_length, Workspace<value_type>& work, value_type* output, size_t stride) const {
        chunk.inflate(work);
    ...
    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& target_indices, const std::vector<Index_>& non_target_indices, Workspace<value_type>& work, value_type* output, size_t stride) const {
        chunk.inflate(work);
...
public:
     ...
     *
     * @tparam accrow_ Whether to extract rows from the chunk, i.e., the rows are the target dimension.
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(Index_ non_target_start, Index_ non_target_length, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_, Index_>(0, core.template get_target_chunkdim<accrow_>(), non_target_start, non_target_length, work.work, output, stride);
    }
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& non_target_indices, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_, Index_>(0, core.template get_target_chunkdim<accrow_>(), non_target_indices, work.work, output, stride);
    }
};
...
public:
    ...
    template<bool accrow_, typename Index_>
    void extract(Index_ non_target_start, Index_ non_target_length, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(0, core.template get_target_chunkdim<accrow_>(), non_target_start, non_target_length, work, output, stride);
    }

    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& non_target_indices, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(0, core.template get_target_chunkdim<accrow_>(), non_target_indices, work, output, stride);
    }
    /**
     * @endcond
     */
...
public:
     ...
     * conversely, if `accrow_ = false`, we would extract a block of columns as the target and the block of rows as the secondary.
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(Index_ target_start, Index_ target_length, Index_ non_target_start, Index_ non_target_length, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(target_start, target_length, non_target_start, non_target_length, work.work, output, stride);
    }
     ...
     * conversely, if `accrow_ = false`, we would extract a block of columns as the target and a subset of rows instead.
     * For a target dimension index `p` and non-target dimension index `non_target_indices[i]`, the value from the chunk should be stored in `output[p * stride + i]`.
     * This layout allows concatenation of multiple chunks into a single contiguous array for easier fetching in the `CustomChunkedMatrix`.
     *
     * Note that implementions of this method do not need to have the exact same template arguments as shown here.
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(Index_ target_start, Index_ target_length, const std::vector<Index_>& non_target_indices, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(target_start, target_length, non_target_indices, work.work, output, stride);
    }
...
public:
     ...
     * conversely, if `accrow_ = false`, we would extract a subset of columns as the target and the block of rows as the secondary.
     * For a target dimension index `p` and non-target dimension index `non_target_start + i`, the value from the chunk should be stored in `output[p * stride + i]`.
     * This layout allows concatenation of multiple chunks into a single contiguous array for easier fetching in the `CustomChunkedMatrix`.
     *
     * Note that implementions of this method do not need to have the exact same template arguments as shown here.
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& target_indices, Index_ non_target_start, Index_ non_target_length, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(target_indices, non_target_start, non_target_length, work.work, output, stride);
    }
     ...
     * This is guaranteed to be greater than or equal to `non_target_indices.size()`.
     *
     * If `accrow_ = true`, we would extract a subset of rows in `target_indices` and a subset columns in `non_target_indices`.
     * conversely, if `accrow_ = false`, we would extract a subset of columns as the target and the subset of rows as the secondary.
     * For a target dimension index `p` and non-target dimension index `non_target_indices[i]`, the value from the chunk should be stored in `output[p * stride + i]`.
     * This layout allows concatenation of multiple chunks into a single contiguous array for easier fetching in the `CustomChunkedMatrix`.
     *
     * Note that implementions of this method do not need to have the exact same template arguments as shown here.
     ...
     */
    template<bool accrow_, typename Index_>
    void extract(const std::vector<Index_>& target_indices, const std::vector<Index_>& non_target_indices, Workspace& work, value_type* output, size_t stride) const {
        core.template extract<accrow_>(target_indices, non_target_indices, work.work, output, stride);
    }
};
